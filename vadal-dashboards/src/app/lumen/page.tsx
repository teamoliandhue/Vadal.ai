  ...
  return (
    <button
      className={`btn-ai group flex items-center gap-3 rounded-full font-semibold ${
        size === "lg" ? "py-2 pl-5 pr-2 text-[13.5px]" : "px-4 py-2 text-[11.5px]"
      } ${className}`}
    >
      {children}
    </button>
    ...
    <aside className="sticky top-0 hidden h-screen w-[236px] shrink-0 flex-col border-r border-line bg-card px-3.5 py-5 text-ink transition-colors lg:flex dark:border-transparent dark:bg-[#101015] dark:text-[#d6d6de] dark:shadow-[1px_0_0_rgba(255,255,255,0.05)]">
      ...
      <button className="mt-5 flex w-full items-center gap-2.5 rounded-xl bg-card p-2.5 text-left ring-1 ring-line shadow-[0_2px_6px_rgba(26,27,31,0.05)] transition hover:bg-soft dark:bg-white/[0.04] dark:shadow-none dark:ring-white/[0.07] dark:hover:bg-white/[0.08]">
        ...
        <span className="min-w-0 flex-1">
          <span className="block truncate text-[13px] font-semibold lowercase text-ink dark:text-white">
            {org.name}
          </span>
          <span className="block truncate text-[10.5px] text-faint dark:text-[#85858f]">
      ...
      <button className="mt-4 flex w-full items-center gap-2.5 rounded-xl bg-[#fdf6e9] px-2.5 py-2.5 text-left ring-1 ring-[#f5e5c2] transition hover:bg-[#fbefd9] dark:bg-transparent dark:ring-0 dark:hover:bg-white/[0.06]">
        ...
        <span className="flex-1 text-[13px] font-semibold text-[#a06a14] dark:text-[#f5cb9b]">
          Today&apos;s AI briefing
        </span>
          ...
          >
            {group.map((it) => (
              ...
              >
                ...
                {it.count && (
                  <span className="rounded-md bg-soft px-1.5 py-0.5 text-[9.5px] font-bold text-muted dark:bg-white/[0.1] dark:text-[#d6d6de]">
                    {it.count}
                  </span>
                )}
              </a>
      ...
      <div className="mt-3 flex items-center gap-2.5 rounded-xl bg-soft px-2.5 py-2.5 ring-1 ring-line dark:bg-white/[0.04] dark:ring-white/[0.07]">
        ...
        <span className="flex-1 text-[12.5px] font-medium text-muted dark:text-[#aaaab4]">
          Health
        </span>
        <span className="text-[13px] font-bold text-ink dark:text-white">{health.score}</span>
        <span className="text-[10.5px] font-semibold text-vgreen dark:text-[#9be3b8]">
          ▲{health.delta}
        </span>
      </div>
      <a
        href="#"
        className="mt-1.5 flex items-center gap-2.5 rounded-[10px] px-2.5 py-2 text-[13px] font-medium text-muted transition hover:bg-soft hover:text-ink dark:text-[#aaaab4] dark:hover:bg-white/[0.05] dark:hover:text-white"
      >
      ...
      {TICKER.map((t, i) => (
        ...
        >
          <div className="min-w-0 flex-1">
            <div className="text-[11px] font-medium text-faint">{t.label}</div>
            <div className="mt-0.5 flex items-baseline gap-1.5">
              <span className="text-[20px] font-bold tracking-tight">{t.value}</span>
              <span className={`text-[11px] font-semibold ${t.good ? "text-vgreen" : "text-vred"}`}>
                {t.delta}
              </span>
            </div>
          </div>
  ...
  return (
    <div className={`card-lift flex flex-col items-center rounded-3xl border border-line bg-card p-7 text-center ${className}`}>
      ...
      <div className="mt-3 flex items-center gap-2">
        <span className="rounded-full bg-vgreen-soft px-2.5 py-1 text-[11px] font-semibold text-vgreen">
          ▲ {health.delta} pts
        </span>
      ...
      <div className="mt-auto w-full pt-5">
        <button className="w-full rounded-full border border-line bg-card py-2.5 text-[12.5px] font-semibold transition hover:bg-soft">
          Why this score →
        </button>
      </div>
    </div>
  ...
  return (
    <div className={`card-lift rounded-3xl border border-line bg-card p-7 ${className}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="text-[15.5px] font-bold tracking-tight">Engagement over time</h3>
          <p className="mt-0.5 text-[12px] text-faint">12 months · vs industry benchmark</p>
        </div>
      ...
      <div className="mt-5 flex items-start gap-2.5 rounded-2xl bg-cream px-4 py-3 ring-1 ring-cream-ring">
        ...
        <p className="text-[12.5px] font-medium leading-relaxed text-cream-ink">
    ...
    <div className={`card-lift relative flex flex-col overflow-hidden rounded-3xl bg-[#141419] p-7 text-white dark:ring-1 dark:ring-white/[0.08] ${className}`}>
      ...
      <div className="relative">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[15.5px] font-bold tracking-tight">Needs your attention</h3>
        </div>
        <ul className="mt-4">
          {aiBriefing.map((b) => (
            <li key={b.text}>
              <button className="group/row -mx-2 flex w-[calc(100%+16px)] items-center gap-3 rounded-xl px-2 py-2.5 text-left transition hover:bg-white/[0.06]">
                ...
                <span className="min-w-0 flex-1">
                  <span className="block text-[13px] font-medium text-zinc-200">{b.text}</span>
                  <span className="block text-[10.5px] text-zinc-500">{b.sub}</span>
                </span>
        ...
        <div className="mt-2 flex items-center justify-between rounded-xl bg-white/[0.05] px-3.5 py-2.5 ring-1 ring-white/[0.07]">
          <span className="text-[10.5px] text-zinc-400">{briefingImpact.label}</span>
          <span className="text-[11px] font-bold text-[#fda4af]">{briefingImpact.value}</span>
        </div>
      </div>
      <div className="relative mt-auto pt-5">
        ...
        <button className="group flex w-full items-center justify-between rounded-full bg-white py-2 pl-6 pr-2 text-[12.5px] font-semibold text-[#1a1b1f] transition hover:bg-zinc-100">
  ...
  return (
    <div className={`card-lift flex flex-col rounded-3xl border border-line bg-card p-7 ${className}`}>
      <div className="flex items-center justify-between">
        <h3 className="text-[15.5px] font-bold tracking-tight">Employee voice</h3>
      ...
      <div className="mt-2.5 flex gap-3.5">
        {voice.mood.map((m) => (
          <span key={m.label} className="flex items-center gap-1.5 text-[10.5px] font-medium text-muted">
          ...
          <li key={t.name} className="group">
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="text-[10.5px] text-faint">{t.mentions} mentions</div>
              </div>
              <span
      ...
      <figure className="mt-5 flex-1 rounded-2xl border border-cream-ring bg-cream px-4 py-3.5">
        <blockquote className="text-[12px] font-medium italic leading-relaxed text-cream-ink">
          <span className="mr-0.5 font-serif text-[15px] not-italic leading-none">“</span>
          {voice.quote.text}
        </blockquote>
        <figcaption className="mt-1.5 text-[10px] font-medium text-faint">
          {voice.quote.meta}
        </figcaption>
      </figure>
      <a href="#" className="mt-4 text-[12px] font-semibold text-vpurple hover:underline">
        Explore all themes →
      </a>
    </div>
  );
...
function ActionQueueCard({ className = "" }: { className?: string }) {
  return (
    <div className={`card-lift flex flex-col rounded-3xl border border-line bg-card p-7 ${className}`}>
      <div className="flex items-center justify-between">
        <h3 className="text-[15.5px] font-bold tracking-tight">Action queue</h3>
      ...
      <div className="mt-3.5 flex items-center gap-3">
        ...
        <span className="text-[10.5px] font-semibold text-muted">{actionProgress.pct}% closure</span>
      </div>
      ...
      <ul className="mt-4 flex-1 space-y-1">
        {actionQueue.map((a, i) => (
          ...
          >
            <span
              className={`mt-1 grid h-5 w-5 shrink-0 place-items-center rounded-md text-[9px] font-bold ${
                a.tone === "urgent"
                  ? "bg-vred-soft text-vred"
                  : a.tone === "warn"
                    ? "bg-vamber-soft text-vamber"
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 text-[10.5px] text-faint">{a.context}</div>
            </div>
            <span
              className={`mt-0.5 shrink-0 rounded-md px-1.5 py-0.5 text-[9px] font-bold tracking-wide ${
                a.due === "Today"
                  ? "bg-vred-soft text-vred"
                  : a.due === "Tomorrow"
                    ? "bg-vamber-soft text-vamber"
      ...
      <button className="mt-3 w-full rounded-full border border-line py-2.5 text-[12.5px] font-semibold transition hover:bg-soft">
        Open manager hub →
      </button>
    </div>
  );
  ...
  return (
    <div className={`card-lift rounded-3xl border border-line bg-card p-7 ${className}`}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h3 className="text-[15.5px] font-bold tracking-tight">Attrition radar</h3>
        <AiButton>
      ...
      <div className="mt-4 flex items-center gap-1 text-[12px]">
        {[
          ...
          <span key={t.label} className="flex items-center">
            ...
            >
              {t.label} <span className="ml-1 text-[10.5px] text-faint">{t.n}</span>
            </button>
        ...
        {flightRisks.map((r) => (
          <li key={r.name} className="group flex cursor-pointer items-center gap-3.5 py-3.5">
            ...
            <div className="w-30 shrink-0">
              ...
              <div className="text-[10.5px] text-faint">{r.team}</div>
            </div>
            <div className="min-w-0 flex-1 truncate text-[12px] text-muted">{r.driver}</div>
            <div className="w-16 shrink-0">
...
function CampaignsCard({ className = "" }: { className?: string }) {
  ...
  return (
    <div className={`card-lift flex flex-col rounded-3xl border border-line bg-card p-7 ${className}`}>
      <div className="flex items-center justify-between">
        <h3 className="text-[15.5px] font-bold tracking-tight">Campaign effectiveness</h3>
        <button className="text-[12px] font-semibold text-vpurple hover:underline">
          New →
        </button>
      </div>
            ...
            <li key={c.name}>
              <div className="flex items-baseline justify-between gap-3">
                <div className="flex min-w-0 items-baseline gap-2">
                  ...
                  <span className="shrink-0 text-[10.5px] text-faint">{c.audience}</span>
                </div>
                <span className="shrink-0 rounded-full bg-vgreen-soft px-2 py-0.5 text-[10px] font-bold text-vgreen">
                  {c.lift} lift
                </span>
              </div>
      ...
      <div className="mt-auto flex items-center gap-2 border-t border-line pt-4 text-[11px] text-muted">
...
function RecognitionCard({ className = "" }: { className?: string }) {
  return (
    <div className={`card-lift flex flex-col rounded-3xl border border-line bg-card p-7 ${className}`}>
      <div className="flex items-center justify-between">
        <h3 className="text-[15.5px] font-bold tracking-tight">Recognition</h3>
      ...
      <div className="mt-5 text-[11px] font-semibold tracking-[0.14em] text-faint">
        TOP APPRECIATORS
      </div>
      <ul className="mb-5 mt-4 space-y-4">
        {recognitionBoard.leaders.map((l, i) => (
          <li key={l.name} className="flex items-center gap-3">
            ...
            />
            <div className="flex-1">
              <div className="text-[13px] font-semibold" style={i === 0 ? warmGrad : undefined}>
                {l.name}
              </div>
      ...
      <div className="mt-auto flex items-start gap-2 rounded-2xl bg-vamber-soft px-4 py-3.5">
        ...
        <p className="text-[11.5px] font-medium leading-relaxed text-[#8a6210] dark:text-[#e3c27e]">
...
function DepartmentsCard({ className = "" }: { className?: string }) {
  return (
    <div className={`card-lift flex flex-col rounded-3xl border border-line bg-card p-7 ${className}`}>
      <div className="flex items-center justify-between">
        <h3 className="text-[15.5px] font-bold tracking-tight">Engagement by department</h3>
        <span className="text-[11px] font-medium text-faint">tap to drill down</span>
      </div>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-x-8 gap-y-3 border-t border-line pt-5">
        ...
        <p className="min-w-0 flex-1 text-[11.5px] font-medium leading-relaxed text-muted">
    ...
    <button className="fixed bottom-5 right-6 z-30 flex items-center gap-2.5 rounded-full border border-line bg-card py-2.5 pl-3 pr-4 shadow-[0_10px_34px_rgba(20,20,25,0.18)] transition hover:-translate-y-0.5 hover:shadow-[0_16px_44px_rgba(139,124,248,0.32)] dark:shadow-[0_10px_34px_rgba(0,0,0,0.5)]">
      ...
      <span className="text-left">
        <span className="flex items-center gap-1 text-[12.5px] font-semibold">
          Vadal
          <span className="rounded-[4px] border border-line px-1 text-[8.5px] font-bold text-muted">
            AI
          </span>
        </span>
