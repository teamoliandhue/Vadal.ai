  ...
  return (
    <section className={`card-lift flex flex-col rounded-[26px] border border-line bg-card p-6 sm:p-7 ${className}`}>
      <div className="flex items-end justify-between">
        <div>
          <p className="text-[12px] font-semibold uppercase tracking-[0.16em] text-faint">Today</p>
          <h2 className="mt-1.5 text-[18px] font-bold tracking-tight">Your day</h2>
        </div>
        <span className="text-[12px] font-medium text-faint">{total > 0 ? `${done}/${total} done` : "Nothing today"}</span>
      </div>
      {total > 0 && (
