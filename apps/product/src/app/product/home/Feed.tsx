    ...
    <section className={`card-lift flex flex-col rounded-[26px] border border-line bg-card p-6 sm:p-7 ${className}`}>
            ...
            {celebrations.map((c) => (
              <div key={c.name} className="flex min-w-[185px] shrink-0 items-center gap-2.5 rounded-2xl border border-line bg-[var(--lav)] p-2.5">
                <span className="relative shrink-0">
                  ...
                  <span className="absolute -bottom-1.5 -right-1.5 text-[14px] leading-none">{c.emoji}</span>
                </span>
      ...
      {ordered.length === 0 ? (
        <div className="mt-5 flex flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-line py-12 text-center">
          ...
          <p className="text-[14px] font-semibold">{empty ? "No posts yet" : "Nothing here yet"}</p>
          <p className="text-[14px] text-faint">{empty ? "Be the first to share something with your team." : "Try another filter, or post an update."}</p>
        </div>
      ) : (
